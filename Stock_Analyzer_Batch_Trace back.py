...
PINNED_COMBOS = [
    ...
    # ⭐⭐ 特別標註：KDJ黃金交叉＋晨星剛形成 家族 ⭐⭐（排最前面）
    # 2026-09-21 3年回測（上市清單）裡最扎實的發現——不是單一天數表現好，是
    # 「同一批樣本、追蹤到5/10/20日三個天數，數字一路遞增」：
    #   純2條件版（n=176）：5日59.1% → 10日61.4% → 20日58.5%
    #   加乖離度負版（n=95）：5日64.2% → 10日67.4% → 20日65.3%
    ["KDJ近3日內黃金交叉", "晨星剛形成"],
    ["KDJ近3日內黃金交叉", "近3月乖離度為負(股價超前營收)", "晨星剛形成"],

    # 以下是「2026-09-21 5/10/20日勝率.xlsx」裡的其他40組，跨5/10/20日三張表
    # 去重後的完整清單（同一組合出現在多張表就合併成一列，註解列出各天數的
    # 勝率）。有些組合彼此是邏輯上的重複（例如同時有「晨星剛形成」跟聚合旗標
    # 「型態成形中／型態突破確認／型態剛形成(剛突破)」，因為晨星是15種型態之一，
    # 晨星成立這些聚合旗標必然也成立，數字會一模一樣，不是抓錯，是刻意保留
    # 讓清單忠於原始Excel）。每列註解的百分比是2026-09-21那次回測的原始紀錄，
    # 不是即時算出來的，之後重新回測數字可能會不一樣。
    ["KDJ近3日內黃金交叉", "三大法人近3月賣超", "晨星剛形成"],  # n=83，5日62.7%，10日63.9%，20日63.9%
    ["KDJ近3日內死亡交叉", "三大法人近3月買超", "一字底(均線糾結)剛形成"],  # n=59，5日61%，10日71.2%
    ["KDJ近3日內黃金交叉", "近3月均價YoY為正", "晨星剛形成"],  # n=107，5日60.7%，10日63.6%，20日61.7%
    ["近3月乖離度為負(股價超前營收)", "三大法人近3月買超", "晨星剛形成"],  # n=97，5日59.8%，20日57.7%
    ["近3月乖離度為正(營收優於股價)", "三大法人近3月賣超", "晨星剛形成"],  # n=59，5日59.3%，20日59.3%
    ["KDJ近3日內黃金交叉", "型態剛形成(剛突破)", "晨星剛形成"],  # n=176，5日59.1%，10日61.4%，20日58.5%（跟純2條件版邏輯重複）
    ["KDJ近3日內黃金交叉", "型態突破確認", "晨星剛形成"],  # n=176，5日59.1%，10日61.4%，20日58.5%（邏輯重複）
    ["KDJ近3日內黃金交叉", "型態成形中", "晨星剛形成"],  # n=176，5日59.1%，10日61.4%，20日58.5%（邏輯重複）
    ["近3月乖離度為負(股價超前營收)", "近3月均價YoY為正", "複式頭肩底剛形成"],  # n=51，5日58.8%
    ["布林通道低檔(≤20%)", "型態突破確認", "三大法人近3月買超"],  # n=102，5日58.8%
    ["布林通道低檔(≤20%)", "型態剛形成(剛突破)", "近3月乖離度為正(營收優於股價)"],  # n=88，5日58%
    ["KDJ近3日內死亡交叉", "三大法人近3月買超", "N字底剛形成"],  # n=100，5日58%
    ["多方力道≥65", "近3月均價YoY為正", "一字底(均線糾結)剛形成"],  # n=104，5日57.7%
    ["布林通道低檔(≤20%)", "型態突破確認", "近3月乖離度為正(營收優於股價)"],  # n=99，5日57.6%
    ["KDJ近3日內黃金交叉", "KDJ近3日內死亡交叉", "晨星剛形成"],  # n=61，5日57.4%
    ["布林通道高檔(≥80%)", "近3月均價YoY為正", "複式頭肩底剛形成"],  # n=56，5日57.1%
    ["強勢突破盤", "KDJ近3日內死亡交叉", "一字底(均線糾結)剛形成"],  # n=58，5日56.9%，10日70.7%
    ["多方力道≥80", "KDJ近3日內死亡交叉", "突破飆股大量黑K最高點剛形成"],  # n=58，5日56.9%
    ["KDJ近3日內死亡交叉", "近3月乖離度為負(股價超前營收)", "一字底(均線糾結)剛形成"],  # n=54，10日66.7%
    ["布林通道高檔(≥80%)", "KDJ近3日內死亡交叉", "一字底(均線糾結)剛形成"],  # n=88，10日65.9%
    ["KDJ近3日內死亡交叉", "一字底(均線糾結)剛形成"],  # n=92，10日63%（死亡交叉卻偏多，留意）
    ["KDJ近3日內死亡交叉", "型態成形中", "一字底(均線糾結)剛形成"],  # n=92，10日63%（邏輯重複）
    ["KDJ近3日內死亡交叉", "型態突破確認", "一字底(均線糾結)剛形成"],  # n=92，10日63%（邏輯重複）
    ["KDJ近3日內死亡交叉", "型態剛形成(剛突破)", "一字底(均線糾結)剛形成"],  # n=92，10日63%（邏輯重複）
    ["KDJ近3日內黃金交叉", "KDJ近3日內死亡交叉", "一字底(均線糾結)剛形成"],  # n=81，10日61.7%
    ["多方力道≥65", "三大法人近3月買超", "一字底(均線糾結)剛形成"],  # n=127，10日60.6%
    ["三大法人近3月買超", "N字底剛形成", "三重底剛形成"],  # n=58，10日60.3%
    ["KDJ近3日內死亡交叉", "近3月均價YoY為負", "一字底(均線糾結)剛形成"],  # n=50，10日60%
    ["強勢突破盤", "KDJ近3日內死亡交叉", "三重底剛形成"],  # n=80，10日60%
    ["KDJ近3日內黃金交叉", "三大法人近3月賣超", "母子懷抱(低檔)剛形成"],  # n=81，20日64.2%
    ["型態剛形成(剛突破)", "圓弧底剛形成", "突破飆股大量黑K最高點剛形成"],  # n=52，20日61.5%，+9.54%（跟純2條件版邏輯重複）
    ["布林通道高檔(≥80%)", "圓弧底剛形成", "突破飆股大量黑K最高點剛形成"],  # n=52，20日61.5%
    ["圓弧底剛形成", "突破飆股大量黑K最高點剛形成"],  # n=52，20日61.5%，+9.54%
    ["型態成形中", "圓弧底剛形成", "突破飆股大量黑K最高點剛形成"],  # n=52，20日61.5%（邏輯重複）
    ["型態突破確認", "圓弧底剛形成", "突破飆股大量黑K最高點剛形成"],  # n=52，20日61.5%（邏輯重複）
    ["近3月均價YoY為正", "母子懷抱(低檔)剛形成", "晨星剛形成"],  # n=58，20日60.3%
    ["近3月均價YoY為正", "三大法人近3月賣超", "晨星剛形成"],  # n=76，20日59.2%
    ["KDJ近3日內黃金交叉", "母子懷抱(低檔)剛形成", "晨星剛形成"],  # n=59，20日57.6%
    ["布林通道低檔(≤20%)", "型態剛形成(剛突破)", "近3月均價YoY為負"],  # n=89，20日57.3%
    ["近3月乖離度為負(股價超前營收)", "近3月均價YoY為正", "晨星剛形成"],  # n=145，20日57.2%
]

# 上面42組在「2026-09-21 5/10/20日勝率.xlsx」原始資料裡實際記錄到的勝率——每組
# 視當初出現在哪幾張表（5/10/20日排行榜），記錄對應的天數跟勝率，不是每組都三個
# 天數齊全（例如只在10日榜出現過的組合，這裡就只會有10的key）。
# key是" ＋ ".join(combo)，用來在指定組合追蹤表裡額外標註「歷史勝率(記錄)」欄位——
# 這是「當時那次回測」的固定數字，不是每次都重新計算，跟pinned_combo_stats()用
# 目前bt_df現算的勝率是兩回事，數字可能會不一樣，用意是留一個歷史記錄參考值
# 方便比對。
PINNED_COMBO_WINRATES = {
    "KDJ近3日內黃金交叉 ＋ 近3月乖離度為負(股價超前營收) ＋ 晨星剛形成": {5: 64.2, 10: 67.4, 20: 65.3},
    "KDJ近3日內黃金交叉 ＋ 三大法人近3月賣超 ＋ 晨星剛形成": {5: 62.7, 10: 63.9, 20: 63.9},
    "KDJ近3日內死亡交叉 ＋ 三大法人近3月買超 ＋ 一字底(均線糾結)剛形成": {5: 61, 10: 71.2},
    "KDJ近3日內黃金交叉 ＋ 近3月均價YoY為正 ＋ 晨星剛形成": {5: 60.7, 10: 63.6, 20: 61.7},
    "近3月乖離度為負(股價超前營收) ＋ 三大法人近3月買超 ＋ 晨星剛形成": {5: 59.8, 20: 57.7},
    "近3月乖離度為正(營收優於股價) ＋ 三大法人近3月賣超 ＋ 晨星剛形成": {5: 59.3, 20: 59.3},
    "KDJ近3日內黃金交叉 ＋ 晨星剛形成": {5: 59.1, 10: 61.4, 20: 58.5},
    "KDJ近3日內黃金交叉 ＋ 型態剛形成(剛突破) ＋ 晨星剛形成": {5: 59.1, 10: 61.4, 20: 58.5},
    "KDJ近3日內黃金交叉 ＋ 型態突破確認 ＋ 晨星剛形成": {5: 59.1, 10: 61.4, 20: 58.5},
    "KDJ近3日內黃金交叉 ＋ 型態成形中 ＋ 晨星剛形成": {5: 59.1, 10: 61.4, 20: 58.5},
    "近3月乖離度為負(股價超前營收) ＋ 近3月均價YoY為正 ＋ 複式頭肩底剛形成": {5: 58.8},
    "布林通道低檔(≤20%) ＋ 型態突破確認 ＋ 三大法人近3月買超": {5: 58.8},
    "布林通道低檔(≤20%) ＋ 型態剛形成(剛突破) ＋ 近3月乖離度為正(營收優於股價)": {5: 58},
    "KDJ近3日內死亡交叉 ＋ 三大法人近3月買超 ＋ N字底剛形成": {5: 58},
    "多方力道≥65 ＋ 近3月均價YoY為正 ＋ 一字底(均線糾結)剛形成": {5: 57.7},
    "布林通道低檔(≤20%) ＋ 型態突破確認 ＋ 近3月乖離度為正(營收優於股價)": {5: 57.6},
    "KDJ近3日內黃金交叉 ＋ KDJ近3日內死亡交叉 ＋ 晨星剛形成": {5: 57.4},
    "布林通道高檔(≥80%) ＋ 近3月均價YoY為正 ＋ 複式頭肩底剛形成": {5: 57.1},
    "強勢突破盤 ＋ KDJ近3日內死亡交叉 ＋ 一字底(均線糾結)剛形成": {5: 56.9, 10: 70.7},
    "多方力道≥80 ＋ KDJ近3日內死亡交叉 ＋ 突破飆股大量黑K最高點剛形成": {5: 56.9},
    "KDJ近3日內死亡交叉 ＋ 近3月乖離度為負(股價超前營收) ＋ 一字底(均線糾結)剛形成": {10: 66.7},
    "布林通道高檔(≥80%) ＋ KDJ近3日內死亡交叉 ＋ 一字底(均線糾結)剛形成": {10: 65.9},
    "KDJ近3日內死亡交叉 ＋ 一字底(均線糾結)剛形成": {10: 63},
    "KDJ近3日內死亡交叉 ＋ 型態成形中 ＋ 一字底(均線糾結)剛形成": {10: 63},
    "KDJ近3日內死亡交叉 ＋ 型態突破確認 ＋ 一字底(均線糾結)剛形成": {10: 63},
    "KDJ近3日內死亡交叉 ＋ 型態剛形成(剛突破) ＋ 一字底(均線糾結)剛形成": {10: 63},
    "KDJ近3日內黃金交叉 ＋ KDJ近3日內死亡交叉 ＋ 一字底(均線糾結)剛形成": {10: 61.7},
    "多方力道≥65 ＋ 三大法人近3月買超 ＋ 一字底(均線糾結)剛形成": {10: 60.6},
    "三大法人近3月買超 ＋ N字底剛形成 ＋ 三重底剛形成": {10: 60.3},
    "KDJ近3日內死亡交叉 ＋ 近3月均價YoY為負 ＋ 一字底(均線糾結)剛形成": {10: 60},
    "強勢突破盤 ＋ KDJ近3日內死亡交叉 ＋ 三重底剛形成": {10: 60},
    "KDJ近3日內黃金交叉 ＋ 三大法人近3月賣超 ＋ 母子懷抱(低檔)剛形成": {20: 64.2},
    "型態剛形成(剛突破) ＋ 圓弧底剛形成 ＋ 突破飆股大量黑K最高點剛形成": {20: 61.5},
    "布林通道高檔(≥80%) ＋ 圓弧底剛形成 ＋ 突破飆股大量黑K最高點剛形成": {20: 61.5},
    "圓弧底剛形成 ＋ 突破飆股大量黑K最高點剛形成": {20: 61.5},
    "型態成形中 ＋ 圓弧底剛形成 ＋ 突破飆股大量黑K最高點剛形成": {20: 61.5},
    "型態突破確認 ＋ 圓弧底剛形成 ＋ 突破飆股大量黑K最高點剛形成": {20: 61.5},
    "近3月均價YoY為正 ＋ 母子懷抱(低檔)剛形成 ＋ 晨星剛形成": {20: 60.3},
    "近3月均價YoY為正 ＋ 三大法人近3月賣超 ＋ 晨星剛形成": {20: 59.2},
    "KDJ近3日內黃金交叉 ＋ 母子懷抱(低檔)剛形成 ＋ 晨星剛形成": {20: 57.6},
    "布林通道低檔(≤20%) ＋ 型態剛形成(剛突破) ＋ 近3月均價YoY為負": {20: 57.3},
    "近3月乖離度為負(股價超前營收) ＋ 近3月均價YoY為正 ＋ 晨星剛形成": {20: 57.2},
}
...
# 飆股搜尋（10日內漲幅超過30%）2026-09-21T02-53那次回測的原始紀錄，去重後
# 共44組，橫跨5/10/20日三張榜單。跟PINNED_COMBOS（分類「高勝率」）是不同
# 目的的兩套清單：那邊看的是整體勝率/平均報酬，這邊看的是「命中大行情的
# 比例」，分類標為「高標股」。
MOONSHOT_COMBOS = [
    ["KDJ近3日內死亡交叉", "回後買上漲全通過", "晨星剛形成"],
    ["多方力道≥65", "回後買上漲全通過", "晨星剛形成"],
    ["KDJ近3日內黃金交叉", "回後買上漲全通過", "晨星剛形成"],
    ["多方力道≥80", "近3月乖離度為負(股價超前營收)", "晨星剛形成"],
    ["MACD近3日內黃金交叉", "KDJ近3日內死亡交叉", "回後買上漲全通過"],
    ["MACD近3日內黃金交叉", "KDJ近3日內死亡交叉", "三重底剛形成"],
    ["多方力道≥80", "近3月均價YoY為正", "晨星剛形成"],
    ["多方力道≥80", "近3月均價YoY為正", "夜星剛形成"],
    ["MACD近3日內黃金交叉", "KDJ近3日內死亡交叉", "K線橫盤的突破剛形成"],
    ["多方力道≥80", "母子懷抱(低檔)剛形成", "晨星剛形成"],
    ["型態成形中", "突破ABC修正下降切線剛形成", "晨星剛形成"],
    ["多方力道≥65", "近3月乖離度為負(股價超前營收)", "晨星剛形成"],
    ["型態突破確認", "突破ABC修正下降切線剛形成", "晨星剛形成"],
    ["回後買上漲全通過", "近3月乖離度為正(營收優於股價)", "晨星剛形成"],
    ["布林通道高檔(≥80%)", "MACD近3日內黃金交叉", "晨星剛形成"],
    ["突破ABC修正下降切線剛形成", "晨星剛形成"],
    ["回後買上漲全通過", "三大法人近3月賣超", "晨星剛形成"],
    ["型態剛形成(剛突破)", "突破ABC修正下降切線剛形成", "晨星剛形成"],
    ["多方力道≥65", "KDJ近3日內黃金交叉", "晨星剛形成"],
    ["MACD近3日內黃金交叉", "三重底剛形成", "K線橫盤的突破剛形成"],
    ["近3月均價YoY為負", "圓弧底剛形成", "突破飆股大量黑K最高點剛形成"],
    ["強勢突破盤", "圓弧底剛形成", "突破飆股大量黑K最高點剛形成"],
    ["MACD近3日內黃金交叉", "三重底剛形成", "圓弧底剛形成"],
    ["三大法人近3月賣超", "圓弧底剛形成", "突破飆股大量黑K最高點剛形成"],
    ["近3月乖離度為正(營收優於股價)", "圓弧底剛形成", "突破飆股大量黑K最高點剛形成"],
    ["多方力道≥80", "三大法人近3月買超", "夜星剛形成"],
    ["KDJ近3日內黃金交叉", "圓弧底剛形成", "突破飆股大量黑K最高點剛形成"],
    ["型態成形中", "圓弧底剛形成", "突破飆股大量黑K最高點剛形成"],
    ["布林通道高檔(≥80%)", "圓弧底剛形成", "突破飆股大量黑K最高點剛形成"],
    ["圓弧底剛形成", "突破飆股大量黑K最高點剛形成"],
    ["型態剛形成(剛突破)", "圓弧底剛形成", "突破飆股大量黑K最高點剛形成"],
    ["型態突破確認", "圓弧底剛形成", "突破飆股大量黑K最高點剛形成"],
    ["多方力道≥80", "近3月均價YoY為負", "母子懷抱(高檔)剛形成"],
    ["多方力道≥65", "圓弧底剛形成", "突破飆股大量黑K最高點剛形成"],
    ["近3月乖離度為負(股價超前營收)", "圓弧底剛形成", "突破飆股大量黑K最高點剛形成"],
...
MOONSHOT_COMBO_STATS = {
    "KDJ近3日內死亡交叉 ＋ 回後買上漲全通過 ＋ 晨星剛形成": { 5: {"n": 21, "moonshot_n": 2, "pct": 9.5, "avg": 37.7} },
    "多方力道≥65 ＋ 回後買上漲全通過 ＋ 晨星剛形成": { 5: {"n": 29, "moonshot_n": 2, "pct": 6.9, "avg": 37.7}, 20: {"n": 29, "moonshot_n": 4, "pct": 13.8, "avg": 59.4} },
    "KDJ近3日內黃金交叉 ＋ 回後買上漲全通過 ＋ 晨星剛形成": { 5: {"n": 29, "moonshot_n": 2, "pct": 6.9, "avg": 37.7} },
    "多方力道≥80 ＋ 近3月乖離度為負(股價超前營收) ＋ 晨星剛形成": { 5: {"n": 33, "moonshot_n": 2, "pct": 6.1, "avg": 32.8} },
    "MACD近3日內黃金交叉 ＋ KDJ近3日內死亡交叉 ＋ 回後買上漲全通過": { 5: {"n": 53, "moonshot_n": 3, "pct": 5.7, "avg": 41.2} },
    "MACD近3日內黃金交叉 ＋ KDJ近3日內死亡交叉 ＋ 三重底剛形成": { 5: {"n": 36, "moonshot_n": 2, "pct": 5.6, "avg": 35.7} },
    "多方力道≥80 ＋ 近3月均價YoY為正 ＋ 晨星剛形成": { 5: {"n": 36, "moonshot_n": 2, "pct": 5.6, "avg": 32.8} },
    "多方力道≥80 ＋ 近3月均價YoY為正 ＋ 夜星剛形成": { 5: {"n": 37, "moonshot_n": 2, "pct": 5.4, "avg": 32.2} },
    "MACD近3日內黃金交叉 ＋ KDJ近3日內死亡交叉 ＋ K線橫盤的突破剛形成": { 5: {"n": 37, "moonshot_n": 2, "pct": 5.4, "avg": 41.1} },
    "多方力道≥80 ＋ 母子懷抱(低檔)剛形成 ＋ 晨星剛形成": { 5: {"n": 20, "moonshot_n": 1, "pct": 5, "avg": 31.7} },
    "型態成形中 ＋ 突破ABC修正下降切線剛形成 ＋ 晨星剛形成": { 5: {"n": 20, "moonshot_n": 1, "pct": 5, "avg": 34}, 10: {"n": 20, "moonshot_n": 2, "pct": 10, "avg": 52.7} },
    "多方力道≥65 ＋ 近3月乖離度為負(股價超前營收) ＋ 晨星剛形成": { 5: {"n": 60, "moonshot_n": 3, "pct": 5, "avg": 32.9} },
    "型態突破確認 ＋ 突破ABC修正下降切線剛形成 ＋ 晨星剛形成": { 5: {"n": 20, "moonshot_n": 1, "pct": 5, "avg": 34} },
    "回後買上漲全通過 ＋ 近3月乖離度為正(營收優於股價) ＋ 晨星剛形成": { 5: {"n": 20, "moonshot_n": 1, "pct": 5, "avg": 41.3} },
    "布林通道高檔(≥80%) ＋ MACD近3日內黃金交叉 ＋ 晨星剛形成": { 5: {"n": 20, "moonshot_n": 1, "pct": 5, "avg": 41.3} },
    "突破ABC修正下降切線剛形成 ＋ 晨星剛形成": { 5: {"n": 20, "moonshot_n": 1, "pct": 5, "avg": 34}, 10: {"n": 20, "moonshot_n": 2, "pct": 10, "avg": 52.7} },
    "回後買上漲全通過 ＋ 三大法人近3月賣超 ＋ 晨星剛形成": { 5: {"n": 20, "moonshot_n": 1, "pct": 5, "avg": 41.3} },
    "型態剛形成(剛突破) ＋ 突破ABC修正下降切線剛形成 ＋ 晨星剛形成": { 5: {"n": 20, "moonshot_n": 1, "pct": 5, "avg": 34} },
    "多方力道≥65 ＋ KDJ近3日內黃金交叉 ＋ 晨星剛形成": { 5: {"n": 41, "moonshot_n": 2, "pct": 4.9, "avg": 37.7} },
    "MACD近3日內黃金交叉 ＋ 三重底剛形成 ＋ K線橫盤的突破剛形成": { 5: {"n": 63, "moonshot_n": 3, "pct": 4.8, "avg": 33.3} },
    "近3月均價YoY為負 ＋ 圓弧底剛形成 ＋ 突破飆股大量黑K最高點剛形成": { 10: {"n": 28, "moonshot_n": 4, "pct": 14.3, "avg": 42.3}, 20: {"n": 28, "moonshot_n": 5, "pct": 17.9, "avg": 62.9} },
    "強勢突破盤 ＋ 圓弧底剛形成 ＋ 突破飆股大量黑K最高點剛形成": { 10: {"n": 45, "moonshot_n": 6, "pct": 13.3, "avg": 39.7}, 20: {"n": 45, "moonshot_n": 8, "pct": 17.8, "avg": 59.7} },
    "MACD近3日內黃金交叉 ＋ 三重底剛形成 ＋ 圓弧底剛形成": { 10: {"n": 32, "moonshot_n": 4, "pct": 12.5, "avg": 44.2} },
    "三大法人近3月賣超 ＋ 圓弧底剛形成 ＋ 突破飆股大量黑K最高點剛形成": { 10: {"n": 24, "moonshot_n": 3, "pct": 12.5, "avg": 37.9} },
    "近3月乖離度為正(營收優於股價) ＋ 圓弧底剛形成 ＋ 突破飆股大量黑K最高點剛形成": { 10: {"n": 25, "moonshot_n": 3, "pct": 12, "avg": 41.5}, 20: {"n": 25, "moonshot_n": 4, "pct": 16, "avg": 65.6} },
    "多方力道≥80 ＋ 三大法人近3月買超 ＋ 夜星剛形成": { 10: {"n": 42, "moonshot_n": 5, "pct": 11.9, "avg": 43.4} },
    "KDJ近3日內黃金交叉 ＋ 圓弧底剛形成 ＋ 突破飆股大量黑K最高點剛形成": { 10: {"n": 26, "moonshot_n": 3, "pct": 11.5, "avg": 35.1}, 20: {"n": 26, "moonshot_n": 4, "pct": 15.4, "avg": 71.8} },
    "型態成形中 ＋ 圓弧底剛形成 ＋ 突破飆股大量黑K最高點剛形成": { 10: {"n": 52, "moonshot_n": 6, "pct": 11.5, "avg": 39.7}, 20: {"n": 52, "moonshot_n": 8, "pct": 15.4, "avg": 59.7} },
    "布林通道高檔(≥80%) ＋ 圓弧底剛形成 ＋ 突破飆股大量黑K最高點剛形成": { 10: {"n": 52, "moonshot_n": 6, "pct": 11.5, "avg": 39.7}, 20: {"n": 52, "moonshot_n": 8, "pct": 15.4, "avg": 59.7} },
    "圓弧底剛形成 ＋ 突破飆股大量黑K最高點剛形成": { 10: {"n": 52, "moonshot_n": 6, "pct": 11.5, "avg": 39.7}, 20: {"n": 52, "moonshot_n": 8, "pct": 15.4, "avg": 59.7} },
    "型態剛形成(剛突破) ＋ 圓弧底剛形成 ＋ 突破飆股大量黑K最高點剛形成": { 10: {"n": 52, "moonshot_n": 6, "pct": 11.5, "avg": 39.7}, 20: {"n": 52, "moonshot_n": 8, "pct": 15.4, "avg": 59.7} },
    "型態突破確認 ＋ 圓弧底剛形成 ＋ 突破飆股大量黑K最高點剛形成": { 10: {"n": 52, "moonshot_n": 6, "pct": 11.5, "avg": 39.7}, 20: {"n": 52, "moonshot_n": 8, "pct": 15.4, "avg": 59.7} },
    "多方力道≥80 ＋ 近3月均價YoY為負 ＋ 母子懷抱(高檔)剛形成": { 10: {"n": 45, "moonshot_n": 5, "pct": 11.1, "avg": 46.2} },
    "多方力道≥65 ＋ 圓弧底剛形成 ＋ 突破飆股大量黑K最高點剛形成": { 10: {"n": 36, "moonshot_n": 4, "pct": 11.1, "avg": 39.4}, 20: {"n": 36, "moonshot_n": 6, "pct": 16.7, "avg": 59.3} },
    "近3月乖離度為負(股價超前營收) ＋ 圓弧底剛形成 ＋ 突破飆股大量黑K最高點剛形成": { 10: {"n": 27, "moonshot_n": 3, "pct": 11.1, "avg": 37.9}, 20: {"n": 27, "moonshot_n": 4, "pct": 14.8, "avg": 53.9} },
    "多方力道≥65 ＋ 母子懷抱(高檔)剛形成 ＋ 夜星剛形成": { 10: {"n": 28, "moonshot_n": 3, "pct": 10.7, "avg": 44.8} },
    "三大法人近3月買超 ＋ 圓弧底剛形成 ＋ 突破飆股大量黑K最高點剛形成": { 10: {"n": 28, "moonshot_n": 3, "pct": 10.7, "avg": 41.5}, 20: {"n": 28, "moonshot_n": 6, "pct": 21.4, "avg": 60.3} },
    "回後買上漲全通過 ＋ 圓弧底剛形成 ＋ 突破飆股大量黑K最高點剛形成": { 10: {"n": 30, "moonshot_n": 3, "pct": 10, "avg": 38.2}, 20: {"n": 30, "moonshot_n": 6, "pct": 20, "avg": 62.7} },
    "強勢突破盤 ＋ 回後買上漲全通過 ＋ 晨星剛形成": { 20: {"n": 25, "moonshot_n": 4, "pct": 16, "avg": 59.4} },
    "近3月均價YoY為負 ＋ 三重底剛形成 ＋ 突破飆股大量黑K最高點剛形成": { 20: {"n": 27, "moonshot_n": 4, "pct": 14.8, "avg": 64.7} },
    "強勢突破盤 ＋ 三大法人近3月賣超 ＋ 晨星剛形成": { 20: {"n": 21, "moonshot_n": 3, "pct": 14.3, "avg": 70.1} },
    "圓弧底剛形成 ＋ 突破飆股大量黑K最高點剛形成 ＋ K線橫盤的突破剛形成": { 20: {"n": 23, "moonshot_n": 3, "pct": 13, "avg": 74.4} },
    "MACD近3日內黃金交叉 ＋ 型態成形中 ＋ 夜星剛形成": { 20: {"n": 31, "moonshot_n": 4, "pct": 12.9, "avg": 50.5} },
    "MACD近3日內黃金交叉 ＋ 型態突破確認 ＋ 夜星剛形成": { 20: {"n": 31, "moonshot_n": 4, "pct": 12.9, "avg": 50.5} },
}
...
def format_moonshot_stats(stats_dict):
    if not stats_dict:
        return None
    parts = [f"{h}日：{format_moonshot_entry(stats_dict[h])}" for h in (5, 10, 20) if h in stats_dict]
    return "　｜　".join(parts) if parts else None


def moonshot_combo_static_stats(combos, stats_map) -> pd.DataFrame:
    """把MOONSHOT_COMBOS固定的44組轉成表格——直接用「2026-09-21T02-53那次回測」
